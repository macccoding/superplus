  ...
  return (
    <div>
      <section className="px-5 pt-6 pb-4">
        ...
        <p className="text-sm text-on-surface-secondary mt-1">Step-by-step guides</p>
      </section>
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            {Array.from(grouped.entries()).map(([category, items]) => (
              <div key={category}>
                <h3 className="text-xs font-bold text-on-surface-secondary uppercase tracking-wide mb-3">{category}</h3>
                <div className="space-y-2">
                  {items.map((g: any) => (
                    <button key={g.id} onClick={() => router.push(`/hub/training/${g.id}`)} className="w-full text-left bg-surface-white rounded-[--radius-lg] p-4 shadow-sm flex items-center gap-4 active:scale-[0.98] transition-all">
                      <div className="w-12 h-12 rounded-[--radius-lg] bg-navy/10 flex items-center justify-center shrink-0">
                        <span className="material-symbols-outlined text-navy" style={{ fontVariationSettings: "'FILL' 1" }}>menu_book</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-on-surface truncate">{g.title}</h4>
                        {g.description && <p className="text-xs text-on-surface-secondary mt-0.5 truncate">{g.description}</p>}
                        <p className="text-xs text-on-surface-secondary mt-1">{g._count.steps} steps</p>
                      </div>
