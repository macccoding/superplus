...
export default function SchedulesAdminPage() {
  ...
  const roleColors: Record<string, string> = {
    ...
    MANAGER: 'bg-tertiary-container/30 text-on-tertiary-container',
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-black text-on-surface">Schedules</h1>
          <p className="text-on-surface-variant mt-1">AI-powered shift scheduling</p>
        </div>
      </div>

      <div className="flex items-center justify-between mb-6 bg-surface-container-lowest rounded-xl p-4 shadow-sm">
        <button onClick={() => setWeekOffset(w => w - 1)} className="w-10 h-10 flex items-center justify-center rounded-lg bg-surface-container-high active:scale-95 transition-all">
        ...
        <div className="text-center">
          ...
          {weekOffset === 0 && <span className="text-xs text-primary font-medium">This Week</span>}
          {schedule && (
            <span className={`text-xs font-bold ml-2 px-2 py-0.5 rounded-full ${
              schedule.status === 'PUBLISHED' ? 'bg-success/10 text-success' : 'bg-tertiary-container/30 text-on-tertiary-container'
      ...
      {isLoading ? (
        ...
        <div className="bg-surface-container-lowest rounded-xl p-6 shadow-sm max-w-xl">
          <div className="text-center mb-6">
            <span className="material-symbols-outlined text-[48px] text-outline mb-2">auto_awesome</span>
            <h2 className="text-lg font-bold text-on-surface">Generate Schedule with AI</h2>
            <p className="text-sm text-on-surface-variant mt-1">Claude will create an optimized schedule based on staff availability and store hours</p>
          </div>
          <textarea
            value={notes}
            ...
            rows={2}
            className="w-full px-4 py-3 bg-surface-container-low border-2 border-outline-variant rounded-xl focus:border-primary focus:outline-none text-on-surface placeholder:text-outline resize-none transition-colors mb-4"
          />
          <button
        ...
        <div>
          ...
          {schedule.status === 'PUBLISHED' && schedule.publishedBy && (
            <div className="bg-success/10 rounded-xl p-4 mb-6 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {weekDates.map((date) => {
              ...
              return (
                <div key={dateKey} className="bg-surface-container-lowest rounded-xl shadow-sm overflow-hidden">
                  <div className="bg-secondary/5 px-5 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-lg font-black text-secondary">{date.getDate()}</span>
                      <span className="font-bold text-on-surface">{DAY_NAMES[date.getDay()]}</span>
                    </div>
                    <span className="text-xs text-on-surface-variant">{daySlots.length} shifts</span>
                  </div>
                  {daySlots.length > 0 ? (
                        ...
                        <div key={slot.id} className="px-5 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-on-surface-variant">{slot.startTime} — {slot.endTime}</p>
                            </div>
                          </div>
                    ...
                    <div className="px-5 py-4 text-sm text-outline text-center">No shifts scheduled</div>
                  )}
                </div>
              );
            })}
