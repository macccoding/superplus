  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => router.push('/admin/orders/new')} className="h-12 px-5 bg-brand text-on-brand font-bold rounded-[--radius-lg] flex items-center gap-2 active:scale-95 transition-all shadow-md">
            ...
            <button key={order.id} onClick={() => router.push(`/admin/orders/${order.id}`)} className="w-full text-left bg-surface-white rounded-[--radius-lg] p-5 shadow-sm active:scale-[0.98] transition-all">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-on-surface-secondary mt-0.5">{order._count.items} items · Created {new Date(order.createdAt).toLocaleDateString()}</p>
                </div>
                {order.totalAmount && (
