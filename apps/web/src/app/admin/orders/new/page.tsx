          ...
          <div className="space-y-2">
            {items.map((item, i) => (
              <div key={i} className="flex gap-2">
                <input value={item.productName} onChange={(e) => { const copy = [...items]; copy[i].productName = e.target.value; setItems(copy); }} placeholder="Product name" className="flex-1 h-12 px-3 bg-surface border-2 border-outline rounded-[--radius-lg] focus:border-primary focus:outline-none text-sm text-on-surface placeholder:text-on-surface-secondary" />
                <input value={item.quantity} onChange={(e) => { const copy = [...items]; copy[i].quantity = e.target.value; setItems(copy); }} placeholder="Qty" type="number" className="w-20 h-12 px-3 bg-surface border-2 border-outline rounded-[--radius-lg] focus:border-primary focus:outline-none text-sm text-on-surface text-center" />
                <input value={item.unitCost} onChange={(e) => { const copy = [...items]; copy[i].unitCost = e.target.value; setItems(copy); }} placeholder="Cost" type="number" className="w-24 h-12 px-3 bg-surface border-2 border-outline rounded-[--radius-lg] focus:border-primary focus:outline-none text-sm text-on-surface text-center" />
            ...
            <button onClick={() => setItems([...items, { productName: '', quantity: '', unitCost: '' }])} className="w-full h-10 border-2 border-dashed border-outline rounded-[--radius-lg] text-sm text-on-surface-secondary font-medium flex items-center justify-center gap-1 hover:border-primary transition-colors">
              <span className="material-symbols-outlined text-[16px]">add</span>Add Item
            </button>
          </div>
        </div>

        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes (optional)" rows={2} className="w-full px-4 py-3 bg-surface border-2 border-outline rounded-[--radius-lg] focus:border-primary focus:outline-none text-on-surface placeholder:text-on-surface-secondary resize-none" />

        <div className="bg-surface-cream rounded-[--radius-lg] p-4 flex items-center justify-between">
          <span className="text-sm text-on-surface-secondary">Estimated Total</span>
