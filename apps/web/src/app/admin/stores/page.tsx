...
export default function StoresPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-black text-on-surface">Stores</h1>
        <p className="text-on-surface-variant mt-1">{stores?.length || 0} locations</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {stores?.map((store) => (
          <div key={store.id} className="bg-surface-container-lowest rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-on-surface-variant mt-0.5">{store.address}</p>
                  <p className="text-sm text-outline mt-0.5">{store.parish}</p>
                </div>
              </div>
