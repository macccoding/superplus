...
const categoryConfig: Record<string, { color: string; icon: string }> = {
  ...
  URGENT: { color: 'bg-brand', icon: 'priority_high' },
  MAINTENANCE: { color: 'bg-warning/20', icon: 'build' },
  INVENTORY: { color: 'bg-navy', icon: 'inventory_2' },
  OTHER: { color: 'bg-outline', icon: 'more_horiz' },
};
...
export function ThreadCard({ title, author, category, messageCount, isPinned, isResolved, updatedAt, onClick }: ThreadCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left bg-surface-white p-4 rounded-[--radius-lg] shadow-[--shadow-card] active:scale-[0.98] transition-all duration-200"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            {isPinned && <span className="material-symbols-outlined text-warning text-[16px]" style={{ fontVariationSettings: "'FILL' 1" }}>push_pin</span>}
            <h3 className={`font-bold text-base truncate ${isResolved ? 'line-through text-outline' : 'text-on-surface'}`}>
              {title}
            </h3>
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-sm text-on-surface-secondary">{author}</span>
            <span className="text-on-surface-secondary">·</span>
            <span className="text-sm text-on-surface-secondary">{messageCount} replies</span>
          </div>
