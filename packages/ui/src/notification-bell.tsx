...
export function NotificationBell({ notifications, unreadCount, onMarkRead, onMarkAllRead, onNavigate }: NotificationBellProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="material-symbols-outlined text-on-surface-variant" style={{ fontVariationSettings: unreadCount > 0 ? "'FILL' 1" : "'FILL' 0" }}>notifications</span>
        {unreadCount > 0 && (
          <span className="absolute top-0.5 right-0.5 min-w-5 h-5 bg-primary text-on-primary text-[10px] font-bold rounded-full flex items-center justify-center px-1">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-12 w-80 max-h-96 bg-surface-container-lowest rounded-xl shadow-xl border border-outline-variant/30 z-50 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant/20">
              <h3 className="font-bold text-on-surface text-sm">Notifications</h3>
              {unreadCount > 0 && (
                <button onClick={() => { onMarkAllRead(); }} className="text-xs text-primary font-medium">
                  Mark all read
                </button>
              )}
            </div>
              ...
              {notifications.length > 0 ? (
                notifications.map((n) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex gap-3 hover:bg-surface-container-low transition-colors ${!n.isRead ? 'bg-primary/5' : ''}`}
                  >
                    <span className={`material-symbols-outlined text-[20px] mt-0.5 ${!n.isRead ? 'text-primary' : 'text-outline'}`}>
                      {typeIcons[n.type] || 'notifications'}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm truncate ${!n.isRead ? 'font-bold text-on-surface' : 'text-on-surface-variant'}`}>{n.title}</p>
